...
function normalizeTopicPayload(form: SaveTopicDto): SaveTopicDto {
  return {
    ...form,
    conceptId: form.conceptId && form.conceptId > 0 ? form.conceptId : undefined,
    category: form.category?.trim() || undefined,
    subCategory: form.subCategory?.trim() || undefined,
    series: form.series?.trim() || undefined,
    tone: form.tone?.trim() || undefined,
    renderStyle: form.renderStyle?.trim() || undefined,
    visualPromptHint: form.visualPromptHint?.trim() || undefined,
...
export default function TopicsPage() {
  ...
  const handleSave = async () => {
    ...
    setDetailLoading(true);
    try {
      const payload = normalizeTopicPayload(form);
      if (selectedId) {
        await topicsApi.update(selectedId, payload);
      ...
      toast.error(getApiErrorMessage(error, "Kayit basarisiz."));
    } finally {
      setDetailLoading(false);
    }
  };
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onChange={(val) =>
                            setForm({
                              ...form,
                              conceptId: val ? Number(val) : undefined,
                            })
                          }
