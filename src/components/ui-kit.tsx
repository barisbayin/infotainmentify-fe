  ...
  return (
    <div
      className={cn(
        ...
        "ui-card relative overflow-hidden rounded-2xl border border-white/5 bg-zinc-900/40 shadow-xl backdrop-blur-2xl transition-all duration-300 hover:border-white/10 hover:shadow-2xl hover:shadow-indigo-500/10",
        noPadding ? "" : "p-6",
        className
      )}
      {...rest}
...
>(({ className, ...props }, ref) => {
  return (
    <input
      ref={ref}
      className={cn(
        "ui-field flex h-11 w-full rounded-xl border border-zinc-800 bg-zinc-950/30 px-3 py-2 text-sm text-white placeholder:text-zinc-600 shadow-sm",
...
>(({ className, ...props }, ref) => {
  return (
    <textarea
      ref={ref}
      className={cn(
        "ui-field flex min-h-[100px] w-full rounded-xl border border-zinc-800 bg-zinc-950/30 px-3 py-2 text-sm text-white placeholder:text-zinc-600 shadow-sm",
...
export function Modal({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] grid place-items-center bg-zinc-950/60 backdrop-blur-sm p-4 animate-in fade-in duration-200 overflow-y-auto">
      ...
      <div
        className={cn(
          "ui-modal-surface relative w-full rounded-2xl border border-zinc-800 bg-zinc-900 shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200",
          widthClasses[maxWidth],
          className // ClassName burada birleştiriliyor
        )}
      >
        {/* Header */}
        <div className="ui-modal-header flex items-center justify-between p-6 border-b border-zinc-800 shrink-0">
          {title && (
  ...
  return (
    <div className={cn("relative w-full", className)} ref={ref}>
      ...
      <button
        ...
        className={cn(
          "ui-select-trigger flex h-9 w-full items-center justify-between rounded-xl border border-zinc-800 bg-zinc-950/30 px-3 text-sm text-zinc-200 transition-all duration-200",
      ...
      {/* Açılır Menü */}
      {isOpen && (
        <div className="ui-select-menu absolute top-full left-0 mt-1 w-full z-50 overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900 shadow-xl animate-in fade-in zoom-in-95 duration-100">
            ...
            {options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "ui-select-option flex w-full items-center justify-between rounded-lg px-2 py-2 text-sm transition-colors",
                    isSelected
...
export function JsonInput({
  ...
  return (
    <div className={cn("relative flex flex-col gap-1 group", className)}>
      {/* EDİTÖR ALANI */}
      <div className="ui-code-field relative flex-1 min-h-0 border border-zinc-800 bg-zinc-950 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500/50 focus-within:border-indigo-500/50 transition-all">
        ...
        {/* Validasyon Rozeti */}
        <div className="ui-floating-badge absolute bottom-3 right-3 pointer-events-none bg-zinc-950/80 backdrop-blur px-2 py-1 rounded border border-zinc-800/50">
          {value && (
            <span
              className={cn(
  ...
  return (
    <div className={cn("flex w-full shadow-sm", className)}>
      ...
      <button
        ...
        className="ui-number-button flex w-9 items-center justify-center rounded-l-xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors focus:outline-none border-r-0 z-10"
      >
        <Minus size={14} />
      </button>
      ...
      <input
        ...
        step={step}
        className={cn(
          "ui-number-input flex-1 h-9 w-full border-y border-zinc-800 bg-zinc-950/50 px-3 text-center text-sm text-zinc-200 placeholder:text-zinc-600",
      ...
      <button
        ...
        className="ui-number-button flex w-9 items-center justify-center rounded-r-xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors focus:outline-none border-l-0 z-10"
      >
        <Plus size={14} />
      </button>
    </div>
...
export function CodeViewer({
  ...
  return (
    <div className={cn("relative group h-full w-full", className)}>
      <div className="ui-code-field absolute inset-0 w-full h-full rounded-xl border border-zinc-800 bg-zinc-950 overflow-hidden">
        <pre
  ...
  return (
    ...
    >
      ...
      {isVisible && (
        <div
          className={cn(
            "ui-tooltip absolute z-50 px-2 py-1 text-xs text-white bg-zinc-900 border border-zinc-800 rounded shadow-lg -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap pointer-events-none",
            className
          )}
        >
          {content}
